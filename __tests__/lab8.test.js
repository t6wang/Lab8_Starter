...
// Create Test Suite (collection of tests executed to test same area of code) for SPA
describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const button = await shadow.asElement().$('button');
    await button.click();
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    expect(parseInt(innerText['_remoteObject'].value)).toBe(20);
  }, 10000);
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    const cartCount = await page.$('#cart-count');
    let innerText = await cartCount.getProperty('innerText');
    expect(parseInt(innerText['_remoteObject'].value)).toBe(0);
